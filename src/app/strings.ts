/**
 * Every user-facing string in the game, in one place, from day one.
 *
 * Not an i18n library, just the single file that makes one possible later. No bare
 * text in components. Ever.
 *
 * Voice rules from §13 of the design doc apply here more than anywhere: bureaucratic not
 * gothic, short, second person, no fantasy nouns. The words this game owns are salt,
 * chalk, ledger, owed, interest, tally, seam, escrow, arrears.
 */
export const strings = {
  brand: {
    title: 'ROUGE',
    tagline: 'You are in the red.',
  },
  pipeline: {
    heading: 'Art pipeline',
    blurb:
      'Placeholders are procedural and keyed to content ID. Drop a PNG at the path under each one, run the manifest, and it appears here with no code change.',
    manifestEmpty: 'Manifest loaded. No art files yet, so everything below is a placeholder.',
    manifestCount: (files: number, kinds: number) =>
      `Manifest loaded: ${String(files)} file${files === 1 ? '' : 's'} across ${String(kinds)} kind${kinds === 1 ? '' : 's'}.`,
    manifestMissing: 'No manifest found. Run `npm run art:manifest`.',
  },
  select: {
    heading: 'The Chalk Wards',
    blurb: 'One act. Twelve nodes. Whatever you are still carrying at the bottom is the bill.',
    seed: 'Seed',
    reroll: 'New seed',
    tier: { normal: 'Debtor', collector: 'Collector', boss: 'Stratum Boss' },
    enter: 'Descend',
    begin: 'Begin the descent',
    resume: 'Resume',
    resumeBlurb: 'A run is still open.',
    abandon: 'Abandon it',
    settings: 'Settings',
    credits: 'Credits',
  },
  settings: {
    heading: 'Settings',
    close: 'Close',
    audioHeading: 'Sound',
    volume: 'Volume',
    mute: 'Mute',
    displayHeading: 'Display',
    reducedMotion: 'Reduced motion',
    reducedMotionHint: 'Cut every animation to a jump cut, on top of whatever your OS already asks for.',
    fontScale: 'Text size',
    fontScaleOption: (percent: number) => `${String(percent)}%`,
    colourblindSafe: 'Colourblind-safe marks',
    colourblindSafeHint: 'Add extra pattern and weight to debt cues, on top of the shape and text already there.',
    playHeading: 'Play',
    fastForwardDefault: 'Fast-forward by default',
    fastForwardDefaultHint: 'Start every fight with animation skipped. F still holds it off either way.',
  },
  credits: {
    heading: 'Credits',
    close: 'Close',
    body: [
      'ROUGE is a roguelike deckbuilding dungeon crawler set in the salt-mining city of Kell Harrow.',
      'Every placeholder on screen is procedural. A separate image pipeline fills in the real art.',
    ],
  },
  tutorial: {
    heading: 'The Tally',
    skip: 'Skip',
    next: 'Next',
    done: 'Understood',
    steps: [
      {
        title: 'Whoever is furthest behind acts next.',
        body: 'There is no turn order. The marker behind on the track always goes first.',
      },
      {
        title: 'Weight is the cost.',
        body: 'Playing a card moves your marker forward by its Weight. A cheap card lets you act again almost immediately.',
      },
      {
        title: 'Hover before you commit.',
        body: 'Hovering a card shows where your marker lands and how many enemy actions happen before you move again.',
      },
      {
        title: 'Guard melts.',
        body: 'Guard decays 1 per beat elapsed, not per turn. Raise it late, close to the hit, and it does more.',
      },
    ],
  },
  run: {
    map: 'The ledger',
    depth: (n: number, of: number) => `Node ${String(n)} of ${String(of)}`,
    salt: 'Salt',
    load: 'Load',
    loadHint: (compounds: number) =>
      compounds === 0
        ? 'Interest falls due at nothing.'
        : `Interest falls due at ${String(compounds)} Compound${compounds === 1 ? '' : 's'} a lap.`,
    marks: 'Marks',
    tokens: 'Collateral',
    deck: 'Deck',
    slots: (used: number, total: number) => `${String(used)} of ${String(total)}`,
    empty: 'Nothing yet.',
    sheet: 'The sheet',
    sheetHint: 'S for the sheet',
    unknown: 'Unread',
    confirm: 'Again to confirm',
    cannotUndo: 'This cannot be taken back.',
    keepAll: 'Keep everything.',
    leave: 'Leave.',
    takeNothing: 'Take nothing.',
    settleHeading: 'Reckoning',
    settleBlurb: 'One card comes off the register and goes onto the sheet. It does not come back.',
    settleNone: 'Nothing here will Settle into anything you do not already carry.',
    removeHeading: 'Strike one off',
    upgradeHeading: 'The ink',
    dipHeading: 'The ink well',
    dipBlurb: 'Darker, heavier, better at what it does.',
    rewardHeading: 'Taken off the body',
    tokenHeading: 'Collateral',
    shopHeading: 'The Assay',
    shopBlurb: 'Pay in Salt, or pay in paper. The scales do not care which.',
    sold: 'Sold',
    wakeHeading: 'A Wake',
    wakeBlurb: 'Nobody is being buried. It is just the only room down here with chairs.',
    priceSalt: (n: number) => `${String(n)} Salt`,
    priceCards: (n: number) => `${String(n)} card${n === 1 ? '' : 's'}`,
    needs: (n: number) => `needs ${String(n)} Salt`,
    settlesInto: (name: string) => `Settles into ${name}`,
    markBurned: 'Its Mark is struck off too, for good.',
    over: { won: 'Discharged', lost: 'In the red' },
    overBlurb: {
      won: 'The act is closed. What is left of you is what you chose to keep.',
      lost: 'It was carried further than it could be carried.',
    },
    overDeckHeading: 'Ended deck',
    overMarksHeading: 'Marks acquired',
    overSettled: (cards: string) => `Settled from ${cards}`,
    overNoMarks: 'No Marks acquired.',
    overNoSettled: 'No Settled card recorded.',
    overCardCount: (name: string, count: number) => `${name} × ${String(count)}`,
    overUnknownCard: (id: string) => `Unread card (${id})`,
    overUnknownMark: (id: string) => `Unread Mark (${id})`,
    again: 'Again',
    walkedNodes: (n: number) => `${String(n)} nodes walked`,
  },
  combat: {
    tally: 'The Tally',
    beat: 'Beat',
    lap: (n: number) => `Lap ${String(n)}`,
    you: 'You',
    hand: 'Hand',
    log: 'The record',
    empty: 'Nothing in hand.',
    waiting: 'Resolving.',
    yourMove: 'Your move.',
    wait: 'Wait a beat',
    waitNoOptions: 'Wait — no card can be played',
    waitFull: 'Clear a Compound to draw',
    weight: 'Weight',
    was: (n: number) => `was ${String(n)}`,
    guard: 'Guard',
    guardHeld: 'Guard (held)',
    strain: 'Strain',
    salt: 'Salt',
    hp: 'HP',
    bleed: 'Bleed',
    draw: 'Draw',
    discard: 'Discard',
    discardZero: 'Discard · 0 Weight',
    discardCompound: (name: string) => `Discard ${name} at 0 Weight`,
    spent: 'Spent',
    marker: (name: string) => `${name}'s marker`,
    guardThrough: (n: number, beat: number) => `Guard ${String(n)}, gone after beat ${String(beat)}`,
    unplayable: 'Unplayable. It just sits there.',
    reink: 'Re-inking',
    phase: (n: number) => `Phase ${String(n)}`,
    dead: 'Settled',
    notaryPhase: (n: number) => `Phase ${String(n)}`,
    reinkWindow: 'Re-ink window',
    reinkIntent: (label: string) => `Re-ink window: ${label}`,
    reinkTag: (multiplier: number, beats: number) => `Re-ink ×${String(multiplier)} · ${String(beats)} beats`,
    reinkActive: (multiplier: number, beats: number) =>
      `Re-ink active: damage ×${String(multiplier)} for ${String(beats)} more beat${beats === 1 ? '' : 's'}`,
    reinkUntil: (beat: number) => `Vulnerability ends after beat ${String(beat)}.`,
    reinkNext: (beat: number, beats: number, multiplier: number) =>
      `Next re-ink: beat ${String(beat)} · ${String(beats)} beats · damage ×${String(multiplier)}.`,
    reinkPattern: (beats: number, multiplier: number) =>
      `Re-ink window: ${String(beats)} beats · damage ×${String(multiplier)}.`,
    countersignCanceled: (lap: number | null, beat: number) =>
      lap === null
        ? `Countersign canceled on beat ${String(beat)}.`
        : `Countersign canceled on lap ${String(lap)} (beat ${String(beat)}).`,
    markStamped: (mark: string, lap: number) => `${mark} stamped on lap ${String(lap)}.`,
    interestDue: (count: number, load: number) =>
      `Interest due: ${String(count)} Compound${count === 1 ? '' : 's'} · Load ${String(load)}.`,
  },
  preview: {
    heading: 'If you play this',
    cost: (weight: number) => `Weight ${String(weight)}`,
    lands: (beat: number) => `lands on beat ${String(beat)}`,
    handsOver: (beats: number) => `${String(beats)} beat${beats === 1 ? '' : 's'} to them`,
    actions: (n: number) => `${String(n)} enemy action${n === 1 ? '' : 's'} first`,
    noActions: 'nothing acts before you do',
    takes: (n: number) => `you take ${String(n)}`,
    takesNothing: 'you take nothing',
    guardLeft: (n: number) => `Guard ${String(n)} left`,
    lapCrossed: 'Interest falls due on the way past',
    kills: (names: readonly string[]) => `kills ${names.join(' and ')}`,
    fatal: 'This kills you.',
    wins: 'This ends it.',
    target: 'Pick a body',
  },
  outcome: {
    won: 'Discharged',
    lost: 'In the red',
    wonBlurb: 'The debt is settled. Nothing is owed on it now.',
    lostBlurb: 'It was carried further than it could be carried.',
    beats: (n: number) => `${String(n)} beats`,
    cards: (n: number) => `${String(n)} cards played`,
    again: 'Again',
    back: 'Back to the Wards',
    onward: 'Go on',
    onwardLost: 'Close the ledger',
  },
  keys: {
    heading: 'Keys',
    close: 'Close',
    help: '? for keys',
    fastForward: 'Fast-forward',
    fastForwardHint: 'hold F',
    lines: [
      ['← →', 'move along the hand'],
      ['1 … 0', 'jump to a card'],
      ['Enter / ↑', 'play it'],
      ['← →', 'pick a body, while targeting'],
      ['Esc', 'back out'],
      ['W', 'wait a beat, draw a card'],
      ['D', 'discard the selected Compound at 0 Weight'],
      ['F', 'hold to fast-forward'],
      ['S', 'the sheet: Marks, collateral, deck'],
      ['Enter', 'go on, once the fight is decided'],
      ['1 … 9', 'jump to a choice, above combat'],
      ['?', 'this list'],
    ],
  },
} as const;
